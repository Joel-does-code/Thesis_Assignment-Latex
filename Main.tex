...
TARGET can be run with future climate models, provided they can deliver a meteorological time series of air temperature, humidity, wind speed, air pressure, and incoming short and longwave radiation. Most future climate models do not provide the temporal resolution (hourly) required to run TARGET, however the study this project was modelled after, \cite{TARGET-ERA5} generated this information by retrofitting minimum, mean, maximum temperatures from the Coupled Model Intercomparison Project Phase 6 (CMIP6) projections \parencite{CMIP6} to the ECMWF (European Centre for Medium-Range Weather Forecasts) ERA5 product, for 2030 and 2050 using emissions scenarios:  \begin{itemize}
    \item 1.2-6: Limit warming below 2°C,
    \item 3.7-0: Middle of the road—current trajectory,
    \item 5.8-5: No policy (worst-case) scenario
\end{itemize}
Land surface cover data will be assumed to be in steady-state.
...
Although TARGET was originally developed and validated for Melbourne \parencite{TARGET}, literature treats Melbourne comparably with Perth \parencite{Perth-Melbourne}. Additionally, it has been shown TARGET is suitable for other urban contexts \parencite{Fast-urban-climate-model-GBI}, as it  builds on the Local-Scale Urban Meteorological Parameterisation Scheme (LUMPS) \parencite{LUMPS} meaning the model captures key surface-atmosphere interactions without the risk of over-parameterisation. TARGET is therefore well suited to short-duration studies and permits the use of heatwave and future climate forcings, enabling consistent comparison across baseline and projected climate conditions. 
\subsection{Property market trends}
Perth has experienced a steady population growth rate over the past several decades \parencite{ABS-data} which has put increased stress on the housing sector \parencite{Curtin-housing-report}. Government responses have centred on rapidly expanding housing supply and increasing residential density around critical infrastructure, e.g., train stations; shopping precints, with current policy ``Perth and Peel @ 3.5 million" \parencite{perth_peel} targeting 47\% infill development and 168,000 new homes per year. One consequence of this shift has been a substantial increase in roof coverage relative to block size in new builds, as the market continues to reward house footprint over backyard space \parencite{Rambo} and consumer sentiment is largely against apartment-living \parencite{high_rises_suck}. 

With limited land releases, the value of land has increased such that larger blocks are prohibitively expensive for many and they are often purchased by redevelopers to become sub-units. Given these options, home-buyers could be contractually committing to an environment with the built form characteristics which lead to high heat exposure. A United States study found the average person below the poverty line lived in areas with elevated UHI compared to those who earn more than two times the poverty line \parencite{Low-income-high-UHI}. In the context of Perth, the concern is these impacts are affecting a larger cohort of people - young and middle-income households - and housing affordability may increasingly function as a mechanism through which thermal risk is distributed across the urban population, though the average home buyer is not considering this element.
%
...
The extent to which other human outcomes are also influenced by increased heat-exposure is discussed in \cite{Social-economic} (CAN I JUST COPY/PASTE THE GRAPHS?) which cites many studies that have found some link between climate variables and social and economic outcomes - increased temperature correlates to increased instances of aggression and conflict (\cite{Hsiang2011-db}; \cite{climate-on-conflict}; \cite{temperature-temperament}; \cite{Crime-weather}); decreased mathematics testing performance (NOT TRUE FOR ENGLISH/READING) (\cite{child-test-scores}), and decreased labour performance in high-risk industries (defined as agriculture, forestry, fishing, and hunting; mining; construction; manufacturing; and transportation and utilities industries.) \parencite{temp-labour}; GDP, productivity and income (\cite{US-income}; \cite{Temp-GDP}; \cite{Temp-production}). While these studies don't have Perth or Australian regions of interest and one could argue that a Perth population must have some higher level of tolerance to heat exposure due to climate adaptation, \parencite{Adjusted-pops} found that mortality rates during days over $35^{\circ}$, the mortality rate increases in all climates. Therefore it is reasonable to consider other heat-related outcomes are not negated by climate-adaptation or preparedness.
...
\subsubsection{Purchasing power scenarios}
Presenting this data will in a user-friendly manner will be critically important in providing meaning to a wide audience. 
